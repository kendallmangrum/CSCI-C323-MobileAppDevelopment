...
public class MonumentFragment extends Fragment {
    ...
    EditText etMonumentName;
    EditText etHistoryOfMonument;
    EditText etTimeToVisit;
    EditText etTicketPrice;
    EditText etMonumentLocation;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        etMonumentName = v.findViewById(R.id.etMonumentName);
        etHistoryOfMonument = v.findViewById(R.id.etMonumentHistory);
        etTimeToVisit = v.findViewById(R.id.etMonumentTimeToVisit);
        etTicketPrice = v.findViewById(R.id.etMonumentHistory);
        etMonumentLocation = v.findViewById(R.id.etMonumentLocation);
...
//      Set an onlclick listener for the add button
        add = v.findViewById(R.id.addMonumentInformationButton);
        add.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {

//                Get all of the information from the editTexts and imageview that the user input
                String monumentName = etMonumentName.getText().toString();
                String monumentTimetoVisit = etTimeToVisit.getText().toString();
                String monumentHistory = etHistoryOfMonument.getText().toString();
                String monumentPrice = etTicketPrice.getText().toString();
                String monumentLocation = etMonumentLocation.getText().toString();
                BitmapDrawable cityImage = (BitmapDrawable) capturedImage.getDrawable();
                Bitmap bitmapCityImage = cityImage.getBitmap();


//              If the user hasn't input any information about the monument and they click add, they are sent to the third activity
                if (monumentName.equals("") && monumentTimetoVisit.equals("") && monumentHistory.equals("") && monumentLocation.equals("")) {
                    startActivity(new Intent(getActivity(), ThirdActivity.class));

//              If the user has input all of the required information, proceed with adding the monument
                } else if (!monumentName.equals("") && !monumentTimetoVisit.equals("") && !monumentHistory.equals("") && !monumentLocation.equals("") && !monumentPrice.equals("")) {
                    File check = new File("data/data/com.c323midtermproject.kmangrum/files/monuments");
                    JSONArray data;

//                    Check to see if file already exists
                    if (check.exists()) {
//                        If it does exist, read the existing file and then add the new monument object to the file
                        try {
                            data = new JSONArray(readJSONDataFromFile("monuments"));

                            JSONObject obj = new JSONObject();
                            obj.put("Name", monumentName);
                            obj.put("Time", monumentTimetoVisit);
                            obj.put("History", monumentHistory);
                            obj.put("Price", monumentPrice);
                            obj.put("Location", monumentLocation);
                            obj.put("Image", getStringFromBitmap(bitmapCityImage));
                            data.put(obj);
                            String dataListUpdated = data.toString();

                            File file = new File("data/data/com.c323midtermproject.kmangrum/files/monuments");
...
//                        If the file doesn't already exist, create the file and store the new monument object there
                    } else {
                        data = new JSONArray();
                        JSONObject obj  = new JSONObject();
                        try {
                        ...
                        try {
                            fos = getContext().openFileOutput("monuments", MODE_APPEND);
...
//                  Provide a toast message that the data has been added and send the user back to the main/first activity
        ...
        cancel = v.findViewById(R.id.cancelMonumentInformationButton);
